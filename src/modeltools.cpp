...
template void
add_transition<MDP>(MDP& mdp, long fromid, long actionid, long outcomeid, long toid, prec_t probability, prec_t reward);
template void add_transition<RMDP_D>(RMDP_D& mdp,
        long fromid,
        long actionid,
        long outcomeid,
        long toid,
        prec_t probability,
        prec_t reward);
template void add_transition<RMDP_L1>(RMDP_L1& mdp,
        long fromid,
        long actionid,
        long outcomeid,
        long toid,
        ...
        for (size_t ai : indices(s)) {
            ...
            if (allowzeros) {
                ...
                for (size_t nsi : indices(probabilities)) {
                    ...
                    Transition& newoutcome = newaction.create_outcome(newaction.size(), probabilities[nsi]);
                ...
                for (size_t nsi : indices(t)) {
                    // create the outcome with the appropriate weight
                    Transition& newoutcome = newaction.create_outcome(newaction.size(), t.get_probabilities()[nsi]);
                    // adds the single sample for each outcome
