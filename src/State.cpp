...
auto SAState<AType>::max_max(numvec const& valuefunction, prec_t discount) const -> tuple<ActionId, OutcomeId, prec_t> {
    if (is_terminal())
...
template <class AType>
auto SAState<AType>::max_min(numvec const& valuefunction, prec_t discount) const -> tuple<ActionId, OutcomeId, prec_t> {
    if (is_terminal())
        return make_tuple(-1, OutcomeId(), 0);
...
template <class AType>
auto SAState<AType>::max_average(numvec const& valuefunction, prec_t discount) const -> pair<ActionId, prec_t> {
    if (is_terminal())
...
template <class AType>
prec_t SAState<AType>::fixed_average(numvec const& valuefunction, prec_t discount, ActionId actionid) const {
    ...
    if (actionid < 0 || actionid >= (long)actions.size())
        throw range_error(
                "invalid actionid: " + to_string(actionid) + " for action count: " + to_string(actions.size()));
...
template <class AType>
prec_t SAState<AType>::fixed_fixed(numvec const& valuefunction,
        prec_t discount,
        ActionId actionid,
        OutcomeId outcomeid) const {
    // this is the terminal state, return 0
    if (is_terminal())
        return 0;
