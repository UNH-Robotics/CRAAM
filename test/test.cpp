...
void test_check_add_transition(typename Model::OutcomeId firstoutcome) {
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(
            transition.get_rewards().begin(), transition.get_rewards().end(), tr.begin(), tr.end());
    vector<double> tp{0.1, 0.5};
    BOOST_CHECK_EQUAL_COLLECTIONS(
            transition.get_probabilities().begin(), transition.get_probabilities().end(), tp.begin(), tp.end());
...
BOOST_AUTO_TEST_CASE(test_robustification) {
    ...
    BOOST_CHECK_CLOSE(mdp.mpi_jac(Uncertainty::Robust, 0.9).valuefunction[0], (1.0 + 2.0) / 2.0, 1e-4);
    BOOST_CHECK_CLOSE(rmdp_nz.mpi_jac(Uncertainty::Robust, 0.9).valuefunction[0], (1.0 + 2.0) / 2.0, 1e-4);
    BOOST_CHECK_CLOSE(rmdp_z.mpi_jac(Uncertainty::Robust, 0.9).valuefunction[0], (1.0 + 2.0) / 2.0, 1e-4);
