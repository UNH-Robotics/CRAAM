...
BOOST_AUTO_TEST_CASE(small_construct_mdpi_r) {
    ...
    Transition initial(vector<long>{0, 1, 2}, vector<prec_t>{1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0}, vector<prec_t>{0, 0, 0});
...
public:
    ...
    */
    ...
    bool end_condition(const State& state) { return false; }

    size_t action_count(State) const { return actions_list.size(); }

    Action action(State, long index) const { return actions_list[index]; }
};
...
BOOST_AUTO_TEST_CASE(implementable_from_samples) {
    ...
    auto sol_impl = mdp->vi_jac_fix(0.9, mdpi.obspol2statepol(isol), indvec(mdp->state_count(), 0));
    ...
    sol_impl = mdp->vi_jac_fix(0.9, mdpi.obspol2statepol(isol), indvec(mdp->state_count(), 0));
...
BOOST_AUTO_TEST_CASE(test_return_of_implementable) {
    ...
    Transition initial1(numvec({1.0, 0.0, 0.0})), initial2(numvec({0.0, 1.0, 0.0})), initial3(numvec({0.0, 0.0, 1.0}));
